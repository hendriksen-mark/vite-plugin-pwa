...
function generateRegisterSW(options2, dev) {
  if (options2.injectRegister === "inline") {
    ...
    return `<script id="vite-plugin-pwa:register-sw" src="${dev ? options2.base : options2.buildBase}${options2.FILE_SW_REGISTER}"${hasDefer ? " defer" : ""}></script>`;
  }
  return void 0;
}
...
function _generateBundle(ctx, bundle) {
  ...
  if ((options2.injectRegister === "script" || options2.injectRegister === "script-defer") && !(0, import_node_fs5.existsSync)((0, import_node_path6.resolve)(viteConfig.publicDir, options2.FILE_SW_REGISTER))) {
    bundle[options2.FILE_SW_REGISTER] = {
      // @ts-expect-error: for Vite 3 support, Vite 4 has removed `isAsset` property
      isAsset: true,
      type: "asset",
      name: void 0,
      source: generateSimpleSWRegister(options2, false),
      fileName: options2.FILE_SW_REGISTER
    };
  }
  return bundle;
}
...
async function createDevRegisterSW(options2, viteConfig) {
  if (options2.injectRegister === "script" || options2.injectRegister === "script-defer") {
    ...
    const registerSW = (0, import_node_path7.resolve)(devDist, options2.FILE_SW_REGISTER);
    ...
    swDevOptions.workboxPaths.set(normalizePath(`${options2.base}${options2.FILE_SW_REGISTER}`), registerSW);
  }
}
    ...
    if (!useImportRegister && injectRegister) {
      ...
      server.ws.send({
        ...
        data: {
          ...
          registerPath: `${base}${options2.FILE_SW_REGISTER}`,
...
async function resolveOptions(ctx) {
  ...
  const {
    ...
    FILE_SW_REGISTER: FILE_SW_REGISTER2 = "registerSW.js",
  ...
  const resolvedVitePWAOptions = {
    ...
    manifestFilename,
    FILE_SW_REGISTER: FILE_SW_REGISTER2,
    strategies,
    workbox,
    manifest,
    useCredentials,
